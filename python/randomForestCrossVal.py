...
if __name__ == "__main__":
    ...
    spark = SparkSession\
        .builder\
        .appName("TrainRandomForestIDS-Python")\
        .getOrCreate()
    ...
    # Create a feature indexer to speed up categorical columns for decision tree
    ...
    auc = BinaryClassificationEvaluator(labelCol="label", rawPredictionCol="prediction", metricName="areaUnderROC").evaluate(predictions)

    maxDepth = bestModel.stages[-1]._java_obj.parent().getMaxDepth()
    numTrees = bestModel.stages[-1]._java_obj.parent().getNumTrees()

    # Stop spark session
    spark.stop()

    file = open("/home/gta/catraca/results/python/csv/randomForestCrossVal.csv", "a+") # Results for MATLAB
...
#               filename += "master-"+coresList[0]+"Cores"
    ...
    file.write(str(f1)+","+str(precision)+","+str(recall)+","+str(accuracy)+","+str(auc)+","+str(modelTime)+","+str(testTime)+","+str(maxDepth)+","+str(numTrees)+"\n")
    file.close()

    file = open("/home/gta/catraca/results/python/"+ dirname +"/"+ filename +"-randomForestCrossVal.txt", "a+")
